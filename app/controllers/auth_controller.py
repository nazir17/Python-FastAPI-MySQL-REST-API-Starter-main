...
@router.post(
    "/signup",
    response_model=response_schema.SingleResponse[user_schema.User],
    responses={
        200: {"model": response_schema.SingleResponse[user_schema.User]},
        400: {"model": response_schema.ErrorResponse},
        422: {"model": response_schema.ValidationErrorResponse},
        500: {"model": response_schema.ErrorResponse},
    },
)
def signup(user: user_schema.UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        raise CustomException(
            message="Email already registered", status_code=status.HTTP_400_BAD_REQUEST
        )
    new_user = auth_service.create_user(db=db, user=user)
    return success_response(
        data=user_schema.User.from_orm(new_user),
...
@router.get(
    "/verify-email",
    response_model=response_schema.SingleResponse[dict],
    responses={
        200: {"model": response_schema.SingleResponse[dict]},
        400: {"model": response_schema.ErrorResponse},
        422: {"model": response_schema.ValidationErrorResponse},
        500: {"model": response_schema.ErrorResponse},
    },
...
@router.post(
    "/signin",
    response_model=response_schema.SingleResponse[user_schema.SignInResponse],
    responses={
        200: {"model": response_schema.SingleResponse[user_schema.SignInResponse]},
        400: {"model": response_schema.ErrorResponse},
        422: {"model": response_schema.ValidationErrorResponse},
        500: {"model": response_schema.ErrorResponse},
    },
...
@router.post(
    ...
    responses={
        200: {"model": response_schema.SingleResponse[dict]},
        404: {"model": response_schema.ErrorResponse},
        422: {"model": response_schema.ValidationErrorResponse},
        500: {"model": response_schema.ErrorResponse},
    },
)
def forgot_password(
    request: user_schema.ForgotPasswordRequest, db: Session = Depends(get_db)
):
...
def reset_password(
    request: user_schema.ResetPasswordRequest, db: Session = Depends(get_db)
):
    user = auth_service.get_user_by_reset_token(db, token=request.reset_token)
    if not user or user.reset_token_expires < datetime.utcnow():
        raise CustomException(
            message="Invalid or expired token", status_code=status.HTTP_400_BAD_REQUEST
        )
