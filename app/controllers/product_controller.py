...
def add_product(product: product_schema.ProductCreate, db: Session = Depends(database.get_db)):
...
def update_product(id: int, product: product_schema.ProductCreate, db: Session = Depends(database.get_db)):
